...
export default class SimpleNavigationHelper {
  ...
  constructor(selector, element, scrollSelector, initialFocusIndex) {
    console.log("SimpleNavigationHelper constructor");
    this.loopable = true;
  ...
  refresh(mutations) {
    ...
    console.log("SimpleNavigationHelper _candidates.length = " + this._candidates.length);
    if (shouldResetFocus ||
      ...
      console.log('SimpleNavigationHelper next = ' + next);
      if (next) {
    ...
    console.log("document.activeElement = " + document.activeElement.id
      + "  this._currentFocus = " + this._currentFocus.id);
  //  if (this.element.contains(document.activeElement) &&
